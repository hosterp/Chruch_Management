...
class DieselPumpLine(models.Model):
	...
	def create(self,vals):
		...
		if vals.get('line_id'):
			rec = self.env['driver.daily.statement'].search([('id','=',vals['line_id'])])
			self.env['fuel.report'].sudo().create({
									'date':rec.date,
									'diesel_pump':result.diesel_pump.id,
									'vehicle_owner':rec.vehicle_no.owner.id,
	...
	def onchange_qty_total_litre_amount(self):
		if self.total_litre_amount != 0:
			if self.per_litre*self.litre != self.total_litre_amount:
				if self.per_litre != 0 and self.litre != 0:
...
class DriverDailyStatement(models.Model):
	...
	vehicle_no = fields.Many2one('fleet.vehicle','Daily Statement Of Vehicle No:', states=READONLY_STATES)
	...
	total_litre_amount = fields.Float('Total Amount', states=READONLY_STATES)
